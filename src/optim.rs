...
pub fn optimize_singles(data: &[SegmentDivergence], parameters: Parameters) -> Result<Vec<f64>> {
    let obs: Vec<Observation> = data
        .iter()
        .map(|s| {
            let theta = 4. * s.mu * parameters.n_scale;
    ...
    if parameters.log_c.num_fit() == 1 {
        ...
        obs.par_iter()
            .map(|o| {
                ...
                let ans = ans.map(|a| parameters.n_scale / a.x.exp());
                ans.wrap_err_with(|| format!("canot optimize for data point {:?}", o))
            })
        ...
        obs.par_iter()
            .map(|o| {
                ...
                let ans = ans.map(|a| a.x * 2. * parameters.n_scale);
                ans.wrap_err_with(|| format!("canot optimize for data point {:?}", o))
            })
            .collect::<Result<Vec<f64>>>()
    } else {
